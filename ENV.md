...
| `JAC_CROSS_ROOT_RESOLVE` | off | Foreign-root GTI resolution, two tiers. (1) Whole-chain: unfiltered, unsliced multi-hop chains resolve entirely in index space via a frontier of `(index, ids)` groups (`TopologyIndex.resolve_chain_cross_root`, ported from `fp-new-main`) -- each hop resolves inside the owner root's index, so 3+ hop chains through foreign territory stay off the walk; one batched foreign-root fetch per hop; foreign index decode goes through gti_cache when `JAC_GTI_CACHE` is on. Any load failure bails to the per-hop path (never undercounts). (2) Per-hop: `_resolve_hop_cross_root`, engages when the whole-chain gate doesn't (predicates, slicing); maps foreign ids through the home root's index each hop, so it falls to the walk once a hop yields nodes the home index has never seen. toml mirror `[run] cross_root_resolve`. |

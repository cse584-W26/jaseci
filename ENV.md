...
|---|---|---|
| `JAC_TOPOLOGY_INDEX` | on | Gates the GTI **write** hooks only (`on_edge_created/destroyed`, `on_node_saved/destroyed`) - reads always use whatever GTI blobs already exist, flag has no read-path effect. **Staleness guard:** an edge mutation while the hooks are off overwrites the affected roots' index blob with a 1-byte stale sentinel (roots with no blob yet get the sentinel too -- otherwise the first hooks-on write would bootstrap a partial index holding only post-dark-era edges); a read that hits the sentinel logs a once-per-root warning; every reader (local `_get_usable_index`, per-hop cross-root, whole-chain frontier, `get_topology_index`) treats the sentinel as "no index" and falls back to the correct edge walk, and the incremental hooks refuse to resume on a stale blob (a partial rebuild would be wrong). Staleness is permanent for that root until an explicit full rebuild (not yet shipped) - toggling the flag off during writes trades away that root's GTI acceleration, never its correctness. Node-only saves while off do not mark stale: a node missing from the index is already caught by the readers' origin/membership guards. |

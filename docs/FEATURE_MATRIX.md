...
| **cross_root** (JAC_CROSS_ROOT_RESOLVE) | **works** | **works** | **works** - resolver-level, backend-agnostic; whole-chain frontier path (topology_index.impl.jac `resolve_chain_cross_root`) for multi-hop chains with predicate-free prefixes (final-hop node predicates apply at the materialization boundary, final-hop edge predicates walk only the last hop), per-hop path (resolver.impl.jac:~39-102) otherwise; on pg-rel bypassed when topology_sql/chain-sql answers first |

...
| **cross_root** (JAC_CROSS_ROOT_RESOLVE) | **works** | **works** | **works** - resolver-level, backend-agnostic; whole-chain frontier path (topology_index.impl.jac `resolve_chain_cross_root`) for unfiltered multi-hop chains, per-hop path (resolver.impl.jac:~39-102) otherwise; on pg-rel bypassed when topology_sql/chain-sql answers first |
